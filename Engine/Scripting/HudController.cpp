...
CREATE(HudController)
{
	...
	SEPARATOR("DEBUG SCREEN");
	MEMBER(MemberType::GAMEOBJECT, mDebugScreen);
	MEMBER(MemberType::GAMEOBJECT, mGodModeBtnGO);
...
void HudController::Start()
{
	...
	if (mDebugScreen)
	{
		mDebugScreen->SetEnabled(false);
		if (mGodModeBtnGO) mGodModeBtn = static_cast<ButtonComponent*>(mGodModeBtnGO->GetComponent(ComponentType::BUTTON));
		if (mGodModeBtn) mGodModeBtn->AddEventHandler(EventType::CLICK, new std::function<void()>(std::bind(&HudController::OnGodModeButtonClick, this)));
	}
...
void HudController::SetScreen(SCREEN name, bool active)
{
	...
	case SCREEN::PAUSE:
		...
		break;
	case SCREEN::DEBUG:
		if (mDebugScreen) mDebugScreen->SetEnabled(active);
		break;
	}
}
...
void HudController::OnGodModeButtonClick()
{
	GameManager::GetInstance()->ActivateGodMode(!GameManager::GetInstance()->IsGodModeActive());

	ImageComponent* buttonImage = static_cast<ImageComponent*>(mGodModeBtnGO->GetComponent(ComponentType::IMAGE));
	if (buttonImage)
	{
		float alpha = (GameManager::GetInstance()->IsGodModeActive()) ? 1.0f : 0.5f;
